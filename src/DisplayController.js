...
import { Project, ToDoItem } from "./ObjectController";
...
const DisplayController = (() => {
  ...
  // State
  let currentlyPopulatedProject = null;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const options = document.querySelectorAll(".dropdown-trigger");
  ...
  const deleteProject = (projects, i) => {
    if (
      currentlyPopulatedProject &&
      currentlyPopulatedProject.getTitle() === projects[i].getTitle()
    ) {
      if (projects.length > 0) {
        populateProject(projects[i - 1]);
      } else {
        MainController.addDefaultProject();
      }
    }
  ...
  const populateProject = (project) => {
    if (project) {
      currentlyPopulatedProject = project;
      clearTaskList();
      ...
      contentHeading.addEventListener("change", () => {
        changeProjectTitle(project, contentHeading);
      });
      addTaskButton = document.querySelector(".addtask");
      addTaskButton.addEventListener("click", () => {
        clickAddTask(project);
      });
      ...
      for (let i = 0; i < project.getItems().length; i++) {
        ...
        itemCheckbox.addEventListener("click", () => {
          clickCheckbox(project.getItems()[i], newContainer, project);
        });
        ...
        newInputField.addEventListener("change", () => {
          changeTitle(project, project.getItems()[i], newInputField.value);
        });
        ...
        itemDueDate.value = format(
          new Date(project.getItems()[i].dueDate),
          "MM/dd/yyyy"
        );
        ...
        itemDueDate.addEventListener("change", () => {
          changeDueDate(project, project.getItems()[i], itemDueDate);
        });
        ...
        lowLink.addEventListener("click", () => {
          clickPriority(
            project.getItems()[i],
            "low",
            colourMark,
        ...
        itemDelete.addEventListener("click", () => {
          clickDelete(newContainer, project, i);
        });
        const deleteIcon = document.createElement("i");
        deleteIcon.classList.add("material-icons");
